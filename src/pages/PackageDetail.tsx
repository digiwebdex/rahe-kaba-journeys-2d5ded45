...
import { Check, ArrowLeft, ArrowRight, Calendar, Clock, MapPin } from "lucide-react";
        ...
        <div className="relative container mx-auto px-4 pt-16 pb-4">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <div className="flex flex-wrap gap-4 text-sm text-muted-foreground">
              {pkg.duration_days && (
                <span className="flex items-center gap-1"><Clock className="h-4 w-4" /> {pkg.duration_days} Days</span>
              )}
              {pkg.start_date && (
                <span className="flex items-center gap-1"><Calendar className="h-4 w-4" /> Starts: {new Date(pkg.start_date).toLocaleDateString()}</span>
              )}
            </div>
          </motion.div>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          ...
          <div className="md:col-span-2 space-y-8">
            {pkg.description && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                <h2 className="font-heading text-xl font-bold mb-3">About This Package</h2>
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="sticky top-24">
            <div className="bg-card border border-border rounded-xl p-6 shadow-luxury">
              ...
              <p className="text-xs text-muted-foreground text-center mt-3">Installment plans available</p>
            </div>
          </motion.div>
        </div>
      </div>
