...
const iconMap: Record<string, any> = { Shield, Star, Plane, MapPin };
...
const HeroSection = () => {
  ...
  const badge = content?.badge || "Trusted Since 2010";
  const headingLine1 = content?.heading_line1 || "Your Sacred";
  const headingLine2 = content?.heading_line2 || "Journey";
  const headingHighlight = content?.heading_highlight || "Begins Here";
  const subheading = content?.subheading || "Premium Hajj & Umrah experiences from Chittagong, Bangladesh. Every detail crafted for your peace of mind.";
  const ctaPrimary = content?.cta_primary || "Explore Packages";
  const ctaSecondary = content?.cta_secondary || "Contact Us";
